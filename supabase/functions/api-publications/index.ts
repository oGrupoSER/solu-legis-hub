/**
 * API endpoint for querying publications
...
serve(async (req) => {
  ...
  try {
    ...
    if (authResult.tokenId && !(await checkRateLimit(authResult.tokenId))) {
      await logRequest(authResult.tokenId, authResult.clientSystemId, '/api-publications', req.method, 429, Date.now() - startTime, req);
      return new Response(
    ...
    if (req.method === 'GET') {
      ...
      if (termo) {
        query = query.contains('matched_terms', [termo]);
      }
      if (diario) {
        query = query.eq('gazette_name', diario);
      }
      if (dataInicial) {
        query = query.gte('publication_date', dataInicial);
      }
      if (dataFinal) {
        query = query.lte('publication_date', dataFinal);
      }
      ...
      await logRequest(authResult.tokenId, authResult.clientSystemId, '/api-publications', req.method, 200, Date.now() - startTime, req);
    ...
    // Method not allowed
    await logRequest(authResult.tokenId, authResult.clientSystemId, '/api-publications', req.method, 405, Date.now() - startTime, req);
    return new Response(
